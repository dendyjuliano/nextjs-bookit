...
import { updateRoom, getRoomDetail, clearErrors } from '../../redux/actions/room.actions'
import { UPDATE_ROOM_RESET } from '../../redux/constants/roomConstans'
...
const UpdateRoom = () => {
    ...
    const { loading: roomDetailsLoading, error: roomDetailsError, rooms } = useSelector(state => state.roomDetails)

    const { id } = router.query

    useEffect(() => {

        if (rooms && rooms._id !== id) {
            dispatch(getRoomDetail('', id))
        } else {
            setName(rooms.name)
            setPrice(rooms.pricePerNight)
            setDescription(rooms.description)
            setAddress(rooms.address)
            setCategory(rooms.category)
            setGuestCapacity(rooms.guestCapacity)
            setNumOfBeds(rooms.numOfBeds)
            setInternet(rooms.internet)
            setBreakfast(rooms.breakfast)
            setAirConditioned(rooms.airConditioned)
            setPetsAllowed(rooms.petsAllowed)
            setRoomCleaning(rooms.roomCleaning)
            setOldImages(rooms.images)
        }
    ...
    const submitHandler = (e) => {
        ...
        dispatch(updateRoom(rooms._id, roomData))
