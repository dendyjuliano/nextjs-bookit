...
import { updateUser, clearErrors } from '../../redux/actions/user.actions'
import { UPDATE_PROFILE_RESET } from '../../redux/constants/userConstans'
...
const Profile = () => {
    ...
    useEffect(() => {
        if (loadedUser) {
            setUser({
                name: loadedUser.user.name,
                email: loadedUser.user.email
            })
            setAvatarPreview(loadedUser.user.avatar.url)
        }
        if (error) {
            toast.error(error)
                    ...
                    <div className="row wrapper">
                        <div className="col-10 col-lg-5">
                            <form className="shadow-lg" onSubmit={submitHandler}>
                                ...
                                >
                                    {updateLoading ? <ButtonLoader /> : 'SUBMIT'}
                                </button>
                            </form>
                        </div>
                    </div>
